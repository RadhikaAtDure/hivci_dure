...
function getColorForsubprovince(e) {
	...
	if(color.iso != undefined){
		if(subprovince.jsondata.indicators[2] != undefined)
		{
			if (iHealthMap.getIndicatorDataType() == "Standard") {
				...
				if(subprovince.jsondata.indicators[2].districtIndicatorDataExt[0] != undefined)
				{
					if (subprovince.jsondata.indicators[2].districtIndicatorDataExt[0][subprovince.currentYear] != undefined)
					{
						color.checkData = subprovince.jsondata.indicators[2].districtIndicatorDataExt[0][subprovince.currentYear][0][color.iso];
					}
				}
			}
		}
	...
	if(color.checkData != undefined){
		...
		if (iHealthMap.getIndicatorDataType() == "Standard") {
			...
			color.data = subprovince.jsondata.indicators[2].districtIndicatorDataExt[0][subprovince.currentYear][0][color.iso][0][0];
		}
...
subprovince.onEachFeature = function(feature,layer){
	...
	if (iHealthMap.getIndicatorDataType() == "Standard") {		
		...
		var mapdata = subprovince.jsondata.indicators[2].districtIndicatorDataExt[0];
	}
	...
	function showsubprovinceInfo(){
		...
		if(subprovince.jsondata.indicators[2] != undefined){
			...
			if (iHealthMap.getIndicatorDataType() == "Standard") {
				...
				if(subprovince.jsondata.indicators[2].districtIndicatorDataExt[0] != undefined){
					subprovinceInfo.data = subprovince.jsondata.indicators[2].districtIndicatorDataExt[0];
				}else{
					console.log("Data is unavailable for this region.");
				}
			}
...
subprovince.setYearRangeAndLimits = function(){
	...
	if (iHealthMap.getIndicatorDataType() == "Standard") {
		...
		subprovince.rangeOfYears = dureUtil.formatObjects(subprovince.jsondata.indicators[2].districtIndicatorDataExt[0]);
	}
